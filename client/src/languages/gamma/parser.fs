...
let whiteAfter w n = n //{ n with WhiteAfter = n.WhiteAfter @ w  }

/// Preppends whitespace beforenode
let whiteBefore w n = n//{ n with WhiteBefore = w @ n.WhiteBefore }
...
let (|Identifier|_|) t = 
  ...
  | white, { Range = rng; Token = (TokenKind.Ident id | TokenKind.QIdent id) } ->
      node rng id |> whiteBefore white |> Some
  | _ -> None
...
let rec precClimb minPrec (OpExpr(app, next)) : Node<Expr> * _ =   
  let rec loop (result:Node<Expr>) (next:(Node<Operator>*OpExpr) option) = 
    match next with 
...
and parsePlaceholder rngLSQuare ctx = 
  ...
  | Some(Identifier id & (_, tokId)) ->
    ...
    | _ ->
        ...
        rng, Expr.Placeholder(node rngLSQuare "", whiteAfter white (node rng Expr.Empty))
...
let parseLetBinding whiteBeforeLet rngLet ctx = 
  ...
  | Some(whiteAfterLet, t) ->
      ...
      Command.Let(whiteBefore whiteAfterLet (node letEndRng "" ), body)
      |> node (unionRanges rngLet body.Range) 
      |> whiteBefore whiteBeforeLet
...
let rec parseCommands acc ctx = 
  ...
  | Some(white, { Token = TokenKind.EndOfFile }) ->
      ...
      match acc with 
      | x::xs -> List.rev (x::xs) // TODO: ({ x with WhiteAfter = white }::xs)
      | [] -> []
